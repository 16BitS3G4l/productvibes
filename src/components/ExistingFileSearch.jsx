...
export function ExistingFileSearch(props) {
  ...
  const [runQuery, {data: pull_data, loading:pull_loading, error:pull_error}] = useLazyQuery(QUERY_GENERAL_FILES);
  const [runQueryForward, {data: pull_data_forward, loading:pull_loading_forward, error:pull_error_forward}] = useLazyQuery(QUERY_GENERAL_FILES_FORWARD);
  const [runQueryBackward, {data: pull_data_backward, loading:pull_loading_backward, error:pull_error_backward}] = useLazyQuery(QUERY_GENERAL_FILES_BACKWARD);
    ...
    if(!pull_loading && !pull_error && pull_data && pull_data.files) {
      ...
      //   console.log("pull " + pull_data)
        ...
        for (var i = 0; i < pull_data.files.nodes.length; i++) {
            ...
            file_url_list.push({url: file, name: filename, id: pull_data.files.nodes[i].id, date_added: new Date(pull_data.files.nodes[i].createdAt).toDateString(), size: pull_data.files.nodes[i].originalFileSize})
  ...
  useEffect(() => {
    ...
    if(!pull_loading_forward && !pull_error_forward && pull_data_forward && pull_data_forward.files) {
      ...
      //   console.log("pull " + pull_data)
        ...
        for (var i = 0; i < pull_data_forward.files.nodes.length; i++) {
            ...
            file_url_list.push({url: file, name: filename, id: pull_data_forward.files.nodes[i].id, date_added: new Date(pull_data_forward.files.nodes[i].createdAt).toDateString(), size: pull_data_forward.files.nodes[i].originalFileSize})
  ...
  useEffect(() => {
    ...
    if(!pull_loading_backward && !pull_error_backward && pull_data_backward && pull_data_backward.files) {
      ...
      //   console.log("pull " + pull_data)
        ...
        for (var i = 0; i < pull_data_backward.files.nodes.length; i++) {
            ...
            file_url_list.push({url: file, name: filename, id: pull_data_backward.files.nodes[i].id, date_added: new Date(pull_data_backward.files.nodes[i].createdAt).toDateString(), size: pull_data_backward.files.nodes[i].originalFileSize})
...
useEffect(() => {
  ...
  items={[
    {
      icon: InfoMinor,
      description: 'Select the files you want, search for any others, and then press continue to connect them to your store\'s resources.',
    },
  ]}
/>
</div>
...
/>
         ...
         <Button disabled={selectedResources.length == 0}>Continue</Button>
        </>
    );
